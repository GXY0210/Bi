...
class CLASSIFIER:
    ...
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20,
                 _batch_size=100, generalized=True, netR=None, dec_size=4096, dec_hidden_size=4096, feature_type='vha'):
        ...
        if self.netR:
            if self.feature_type == 'a':
            ...
            elif self.feature_type == 'ha':
                self.netR.eval()
                print('Classifier input feature type is HA !!!!!')
                self.input_dim = dec_size
                self.input_dim += dec_hidden_size
            else:
                ...
                self.input_dim += dec_hidden_size  # 拼接三种模态
            self.model = LINEAR_LOGSOFTMAX_CLASSIFIER(self.input_dim, self.nclass)
            ...
            print('Classifier input feature type is V !!!!!')
            self.model = LINEAR_LOGSOFTMAX_CLASSIFIER(self.input_dim, self.nclass)
            self.train_X = self.compute_dec_out(self.train_X, self.input_dim)
            self.test_unseen_feature = self.compute_dec_out(self.test_unseen_feature, self.input_dim)
            self.test_seen_feature = self.compute_dec_out(self.test_seen_feature, self.input_dim)
        ...
        if generalized:
            ...
            sim = syn_att @ un_att.transpose(0, -1)  # 计算相似度
            predict = torch.argmax(sim, dim=1).cpu()  # 相似度最高的做预测结果
            label = torch.tensor(data_loader.test_unseen_mapped_label)  # 获取真实标签
            acc = torch.sum(predict == label) / len(label)  # 计算准确率
            # print(predict)
            # print(label)
            frequency = predict.bincount() / len(label)  # 计算每个分类的预测概率
    ...
    def val(self, test_X, test_label, target_classes):
        ...
        acc, per_acc = self.compute_per_class_acc(util.map_label(test_label, target_classes), predicted_label,
    ...
    def compute_dec_out(self, test_X, new_size):
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.feature_type == 'a':
                ...
                new_test_X[start:end] = torch.cat([inputX], dim=1).data.cpu()
            elif self.feature_type == 'ha':
                new_test_X[start:end] = torch.cat([feat1, feat2], dim=1).data.cpu()
            else:

...
sys.path.append('/home/staff_xiaobo_jin/prml/XinyuanRu/ZSL/Bi-VAEGAN-master')
...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        self.real_class_prior = unseen_class_counts / unseen_class_counts.sum()  # 计算真实的不可见类的概率
        ...
        self.seen_att = self.attribute[self.seenclasses]  # 在 CE 中叫attribute_seen
    ...
    def next_unseen_batch(self, seen_batch, perb=False, unknown_prior=False, unseen_prior=None, ):
        ...
        if not unknown_prior:  # 随机采样
            ...
            if unseen_prior is not None:  # 已知先验采样
    ...
    def norm_feature(self, radius=1):
        ...
        self.test_unseen_feature = radius * self.test_unseen_feature / torch.norm(self.test_unseen_feature, p=2, dim=1,
                                                                                  keepdim=True)
        self.test_seen_feature = radius * self.test_seen_feature / torch.norm(self.test_seen_feature, p=2, dim=1,
                                                                              keepdim=True)
